...
from sqf.common_expressions import TryCatchExpression, ForEachExpressionArray, ForEachExpressionHashMap, \
...
def _foreach_loop(interpreter, code, elements):
    ...
    if type(elements) is dict:
        for i, (x, y) in enumerate(elements.items()):
            outcome = interpreter.execute_code(code, extra_scope={'_x': x, '_y': y, '_forEachIndex': Number(i)})
    else:
...
INTERPRETER_EXPRESSIONS = [
    ...
    ForEachExpressionArray(lambda lhs, rhs, i: _foreach_loop(i, lhs, rhs.value)),
    ForEachExpressionHashMap(lambda lhs, rhs, i: _foreach_loop(i, lhs, rhs.value)),
