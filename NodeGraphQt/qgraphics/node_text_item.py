...
# CUSTOM NODE TEXT ITEM FOR BACKDROP

class NodeTextItemForBackdrop(QtWidgets.QGraphicsTextItem):
    """
    NodeTextItem class used to display and edit the name of a NodeItem.
    """

    def __init__(self, text, parent=None):
        super(NodeTextItemForBackdrop, self).__init__(text, parent)
        self._locked = False
        self._canEdit = True #custom for disable edit mode
        self.set_locked(False)
        self.set_editable(False)

        self._isEditProcess = False
        #self.ItemUsesExtendedStyleOption = True
        #self.setTextWidth(50) #multiline
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() == QtCore.Qt.Key_Return:
            ...
            self.setPlainText(self.node.name)
            self.set_editable(False)
        super(NodeTextItemForBackdrop, self).keyPressEvent(event)
    ...
    def focusOutEvent(self, event):
        ...
        self.set_node_name(current_text)
        self.set_editable(False)
        super(NodeTextItemForBackdrop, self).focusOutEvent(event)
    ...
    def set_node_name(self, name):
        if name != self.node.name:
            ...
            undo_cmd.setText(
                'Переименован "{}" в "{}"'.format(pname, name)
            )
