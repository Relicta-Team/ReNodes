...
class NodeViewer(QtWidgets.QGraphicsView):
    ...
    def dropEvent(self, event):
        ...
        if event.source() and isinstance(event.source(),QtWidgets.QTreeWidget):
            if event.source().objectName() == 'VariableManager.tree':
                ...
                actList = graphRef.get_context_menu('context').prepareActions("addVariable",contextData,fmtText=curitem.text(0))
                
                if len(actList) == 1:
                    actList[0]._on_triggered()
                    return
    ...
    def _validate_accept_connection(self, from_port, to_port):
        ...
        if constraints:
            if to_port.port_typeName in accepted_pnames or not to_port.port_typeName: #Yobas: replaced from to_port.name (#15)
    ...
    def establish_connection(self, start_port, end_port):
        ...
        curPort = start_port if start_port.port_typeName != "" else end_port
        pipe = PipeItem(color=curPort.color)
