...
class CGFileLockedError(CGBaseException):
    id = 8
    text = "Невозможно получить доступ к файлу"
    desc = "Не удалось получить доступ к файлу \"{ctx}\", т.к. он заблокирован другим процессом (вероятнее всего Платформой)"
    moreInfo = "Данное исключение возникает, когда ReNode пытается записать или удалить какие-то данные в файл, который в данный момент Платформа (или другое приложение) открыло для изменений. Переключитесь на окно платформы, закройте все открытые месседжбоксы в платформе и повторите попытку."
...
class CGSuperVoidReturnException(CGBaseException):
    id = 313
    text = "{src} не может возвращать значение"
    desc = "Точка входа, в которой используется {src} не имеет возвращаемого значения. Уберите его выходное подключение к порту \'Значение\' из {src}."
    moreInfo = "Если точка входа не возвращает значения, то порт узла {src} не может иметь никаких входных данных. Оставьте его пустым без каких-либо подключений."
...
class CGEntryFunctionSignatureException(CGBaseException):
    id = 319
    text = "Неверный тип данных в {src}"
    desc = "Ошибка при обработке сигнатуры функции {src}"
    moreInfo = "Дополнительная информация: {ctx}"

class CGEntryLocalFunctionInvalidNode(CGBaseException):
    id = 320
    text = "Недопустимый узел {src} в {entry}"
    desc = "Узел {src} не допускается к использованию в контексте точки входа {entry}"
    moreInfo = "Некоторые узлы невозможно использовать внутри локальных функций. Уберите из точки входа {entry} узел {src}"
...
class CGEntrySelfObjectRefUnsupported(CGBaseException):
    id = 322
    text = "Недопустимое использование {src} в контексте {entry}"
    desc = "В {entry} не содержится ссылка на объект. Требуется явная передача объекта из выходных портов {entry}."
    moreInfo = "Локальные функции, лямбды и события не содержат ссылку на объект графа и локальные переменные, так как контекст выполнения не передается в них неявно."

class CGEntryLambdaSuperNotSupported(CGBaseException):
    id = 323
    text = "Невозможное использование базового метода внутри {entry}"
    desc = "Точка входа {entry} является лямбдой, делегатом или локальной функцией и не имеет базовых методов."
    moreInfo = "Уберите связь {src} из точки входа {entry}"
