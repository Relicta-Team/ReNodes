...
class NodeFactory:
	...
	def addProperty(self,node,type,optname,optvals):
		...
		if type=='typeselect':
			node.add_typeselect(name=optname,label=optvals.get('text',''),value=optvals.get('default','object'),typeset_out=optvals.get('typeset_out'),port_rename=optvals.get('port_rename'),restrict_bydefault=optvals.get('restr_def',True))
