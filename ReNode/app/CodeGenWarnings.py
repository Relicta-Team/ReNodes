...
# ----------------------------------------
class CGLambdaRefNotUsedWarning(CGBaseWarning):
    id = 101
    text = "Неиспользуемая пользовательская функция {src}"
    desc = "{src} создается, но нигде не используется. Удалите или подключите выходной порт \"{portname}\" узла {src}."
# ----------------------------------------
...
class CGFunctionRecursionWarning(CGBaseWarning):
    id = 306
    text = "Вероятная рекурсия вызова {src}"
    desc = "Точка входа {entry} вызывает саму себя в {src}, что может привести при неправильной обработке к бесконечному циклу или полному зависанию платформы."

class CGLocalFunctionNotUsed(CGBaseWarning):
    id = 307
    text = "Неиспользуемая пользовательская функция {src}"
    desc = "Пользовательская функция, определенная в графе нигде не используется."
